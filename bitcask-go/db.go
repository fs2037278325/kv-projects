...
func Open(options Options) (*DB, error) {
	...
	db := &DB{
		options:    options,
		mu:         new(sync.RWMutex),
		olderFiles: make(map[uint32]*data.DataFile),                                          // 初始化旧数据文件
		index:      index.NewIndexer(options.IndexType, options.DirPath, options.SyncWrites), // 初始化内存索引
		isInitial:  isInitial,
		fileLock:   fileLock,
	}
...
func (db *DB) loadDataFiles() error {
	...
	for _, entry := range dirEntries {
		if strings.HasSuffix(entry.Name(), data.DataFileNameSuffix) {
			// 0001.data， 分割获取到前面的fileId，0001.data是一种约定俗成的文件命名，文件id递增，后缀.data
			splitNames := strings.Split(entry.Name(), ".")
			fileId, err := strconv.Atoi(splitNames[0])
			// 数据目录有可能被损坏了
			if err != nil {
				return ErrDataDiretoryCorrupted
			}
